...
class DamagePainter extends CustomPainter {
  ...
  void _drawLabel(Canvas canvas, Rect box, String label, double score) {
    final text = ' $label - ${(score * 100).toInt()}% ';
    ...
    strokePainter.paint(canvas, Offset(box.left, labelY));

    fillPainter.paint(canvas, Offset(box.left, labelY));
  }
  ...
  Color _getColorForDamage(String label) {
    ...
    if (label.contains('D10')) return Colors.yellow; // Medium
    if (label.contains('D00')) return Colors.green; // Minor
    return Colors.white; 
  }
